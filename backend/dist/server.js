...
function cleanUsername(input) { return String(input ?? "").trim().replace(/^@/, "").toLowerCase(); }
function validUsername(username) { return /^[a-z0-9_]{2,24}$/.test(username); }
function cleanText(input, max) { return String(input ?? "").trim().slice(0, max); }
function tokenFrom(req) { return req.header("authorization")?.startsWith("Bearer ") ? req.header("authorization").slice(7).trim() : ""; }
function asNumber(value) { return Number(value); }
function imageUrl(imageId) { return imageId ? `/api/images/${imageId}` : null; }
function userObject(row) { return { id: asNumber(row.id), username: row.username }; }
...
async function saveImage(file) {
    ...
    if (!new Set(["image/jpeg", "image/png", "image/webp", "image/gif"]).has(file.mimetype))
...
async function deleteImageIfUnused(id) {
    if (!id)
        return;
...
async function cleanupStories() {
    const expired = await db.query("DELETE FROM stories WHERE expires_at <= $1 RETURNING image_id AS \"imageId\"", [now()]);
    await Promise.all(expired.rows.map(story => deleteImageIfUnused(story.imageId)));
}
function postObject(row) {
...
app.get("/api/images/:id", async (req, res, next) => {
    try {
        const image = await db.query("SELECT id, mime_type AS \"mimeType\", data FROM images WHERE id = $1", [req.params.id]);
...
app.post("/api/auth/register", async (req, res, next) => {
    try {
        ...
        const createdAt = now();
        const created = await db.query("INSERT INTO users (username, created_at) VALUES ($1, $2) RETURNING id::text AS id, username", [username, createdAt]);
...
app.post("/api/auth/access", async (req, res, next) => {
    try {
        ...
        const found = await db.query("SELECT id::text AS id, username FROM users WHERE username = $1", [username]);
        if (!found.rows[0])
            return res.status(404).json({ error: "No account with that username exists." });
        const user = userObject(found.rows[0]);
        const token = crypto.randomBytes(32).toString("hex");
        const timestamp = now();
...
app.post("/api/auth/logout", authenticate, async (req, res, next) => { try {
    await db.query("DELETE FROM sessions WHERE token = $1", [tokenFrom(req)]);
    res.json({ ok: true });
}
catch (error) {
    next(error);
...
app.post("/api/posts", authenticate, upload.single("image"), async (req, res, next) => {
    try {
        ...
        if (!content && !req.file)
            return res.status(400).json({ error: "Write something or attach an image." });
        const imageId = await saveImage(req.file);
        const created = await db.query("INSERT INTO posts (user_id, content, image_id, created_at) VALUES ($1, $2, $3, $4) RETURNING id::text AS id", [req.user.id, content, imageId, now()]);
...
app.delete("/api/posts/:id", authenticate, async (req, res, next) => {
    try {
        ...
        const deleted = await db.query("DELETE FROM posts WHERE id = $1 AND user_id = $2 RETURNING image_id AS \"imageId\"", [id, req.user.id]);
        if (!deleted.rows[0])
            return res.status(404).json({ error: "Post not found or not owned by you." });
        await deleteImageIfUnused(deleted.rows[0].imageId);
        res.json({ ok: true, id });
    }
    catch (error) {
        next(error);
...
app.post("/api/posts/:id/replies", authenticate, async (req, res, next) => {
    try {
...
app.delete("/api/replies/:id", authenticate, async (req, res, next) => { try {
    const deleted = await db.query("DELETE FROM replies WHERE id = $1 AND user_id = $2 RETURNING id", [req.params.id, req.user.id]);
    if (!deleted.rows[0])
        return res.status(404).json({ error: "Reply not found or not owned by you." });
    res.json({ ok: true, id: Number(req.params.id) });
}
catch (error) {
    next(error);
} });
app.get("/api/stories", authenticate, async (_req, res, next) => { try {
    await cleanupStories();
    const stories = await db.query(`SELECT ${storyFields} FROM stories s JOIN users u ON u.id = s.user_id WHERE s.expires_at > $1 ORDER BY s.created_at DESC`, [now()]);
    res.json({ stories: stories.rows.map(storyObject) });
}
catch (error) {
    next(error);
} });
app.post("/api/stories", authenticate, upload.single("image"), async (req, res, next) => { try {
    const content = cleanText(req.body?.content, 600);
    if (!content && !req.file)
        return res.status(400).json({ error: "Write something or attach an image." });
    const timestamp = now();
    const imageId = await saveImage(req.file);
    const created = await db.query("INSERT INTO stories (user_id, content, image_id, created_at, expires_at) VALUES ($1, $2, $3, $4, $5) RETURNING id::text AS id", [req.user.id, content, imageId, timestamp, timestamp + STORY_LIFETIME]);
    const story = await db.query(`SELECT ${storyFields} FROM stories s JOIN users u ON u.id = s.user_id WHERE s.id = $1`, [created.rows[0].id]);
    res.status(201).json({ story: storyObject(story.rows[0]) });
}
catch (error) {
    next(error);
} });
app.delete("/api/stories/:id", authenticate, async (req, res, next) => { try {
    const deleted = await db.query("DELETE FROM stories WHERE id = $1 AND user_id = $2 RETURNING image_id AS \"imageId\"", [req.params.id, req.user.id]);
    if (!deleted.rows[0])
...
app.get("/api/users/:username", authenticate, async (req, res, next) => { try {
    const username = cleanUsername(req.params.username);
    const user = await db.query("SELECT id::text AS id, username, created_at::text AS \"createdAt\" FROM users WHERE username = $1", [username]);
    if (!user.rows[0])
        return res.status(404).json({ error: "User not found." });
    const posts = await db.query(`SELECT ${postFields} FROM posts p JOIN users u ON u.id = p.user_id WHERE p.user_id = $1 ORDER BY p.created_at DESC`, [user.rows[0].id]);
    res.json({ user: { ...userObject(user.rows[0]), createdAt: asNumber(user.rows[0].createdAt) }, posts: posts.rows.map(postObject) });
}
catch (error) {
    next(error);
} });
app.get("/api/search", authenticate, async (req, res, next) => { try {
    const q = cleanText(req.query.q, 40);
    if (!q)
        return res.json({ users: [], posts: [] });
    const pattern = `%${q}%`;
    const [users, posts] = await Promise.all([db.query("SELECT id::text AS id, username FROM users WHERE username ILIKE $1 ORDER BY username ASC LIMIT 20", [pattern]), db.query(`SELECT ${postFields} FROM posts p JOIN users u ON u.id = p.user_id WHERE p.content ILIKE $1 OR u.username ILIKE $1 ORDER BY p.created_at DESC LIMIT 30`, [pattern])]);
    res.json({ users: users.rows.map(userObject), posts: posts.rows.map(postObject) });
}
catch (error) {
    next(error);
} });
app.use((error, _req, res, _next) => {
    if (error?.code === "LIMIT_FILE_SIZE")
        return res.status(400).json({ error: "Image must be smaller than 99 KB." });
    console.error(error);
    res.status(500).json({ error: "The API could not complete this request." });
});
